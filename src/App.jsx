...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body { font-family: 'OCR A Extended', monospace; font-size: 11pt; color: #111; line-height: 1.6; padding: 40px; max-width: 800px; margin: 0 auto; }
                ...
                {filteredInvoices.map((inv) => (
                  <div key={inv.id} className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      ...
                      {(inv.status === 'PAID' || inv.paid > 0) && <button onClick={() => generateReceipt(inv)} className="bg-green-500 text-white px-3 py-2 rounded-lg hover:bg-green-600 transition text-xs font-medium flex items-center gap-1"><Receipt size={14} /> Receipt</button>}
                      ...
                      <tr key={inv.id} className="border-t border-gray-200">
                        ...
                        <td className="text-center p-3 md:p-4">
                          <div className="flex gap-2 justify-center flex-wrap">
                            ...
                            {(inv.status === 'PAID' || inv.paid > 0) && <button onClick={() => generateReceipt(inv)} className="bg-green-500 text-white p-2.5 rounded-lg hover:bg-green-600 transition shadow-sm" title="Generate Receipt"><Receipt size={18} /></button>}
                            {inv.paymentHistory && inv.paymentHistory.length > 0 && (
                              <button onClick={() => { const last = inv.paymentHistory[inv.paymentHistory.length - 1]; generateReceipt(inv, last); }} className="bg-purple-500 text-white p-2.5 rounded-lg hover:bg-purple-600 transition shadow-sm" title="Generate Last Payment Receipt"><Receipt size={18} /></button>
                            )}
